...
export default function FormulariosPage() {
  ...
  const generateTxtFile = (form: Formulario) => {
    ...
    if(d.responsibleCpf) content += `CPF DO RESPONSÁVEL: ${d.responsibleCpf.replace(/\D/g, '')}\n`;

    content += `\n2. DOCUMENTAÇÃO\n`;
    content += `----------------\n`;
    content += `CPF: ${form.cpf ? form.cpf.replace(/\D/g, '') : '-'}\n`;
    content += `RG: ${d.rg || '-'}\n`;
    ...
    content += `NÚMERO: ${d.addressNumber || '-'}\n`;
    if(d.complement) content += `COMPLEMENTO: ${d.complement}\n`;
    content += `BAIRRO: ${d.neighborhood || '-'}\n`;
    content += `CIDADE: ${d.city || '-'}\n`;
    content += `ESTADO: ${d.state || '-'}\n`;
    content += `CEP: ${d.zipCode ? d.zipCode.replace(/\D/g, '') : '-'}\n`;
    content += `TELEFONE: ${d.phone ? d.phone.replace(/\D/g, '') : '-'}\n`;
      ...
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="border rounded-lg p-4 bg-gray-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <span className="text-gray-500">Número:</span>
              <p className="font-medium">{dados.addressNumber || '-'}</p>
            </div>
            {dados.complement && (
              <div className="sm:col-span-2 break-words">
                <span className="text-gray-500">Complemento:</span>
                <p className="font-medium">{dados.complement}</p>
              </div>
            )}
            <div>
